...
export const SelectLabel = props => (
  <div style={{ position: 'absolute', marginTop: '34px', whiteSpace: 'nowrap' }}>
    <label className="mdc-textfield__label mdc-textfield__label--float-above">{props.children}</label>
  </div>
);
...
export class Select extends MDCComponentBase {
  ...
  MDCHandleProps(nextProps, initialMount) {
    if (this.props.value !== nextProps.value || initialMount) {
      const newIndex = this.MDCApi.options.indexOf(this.MDCApi.nameditem(nextProps.value));
  ...
  render() {
    ...
    const displayValue = this.getDisplayValue(value, selectOptions, placeholder);
