...
export const DialogTemplate = props => {
  const { title, header, body, footer, acceptLabel, cancelLabel, children, ...rest } = props;
    ...
    <DialogRoot {...rest}>
      <DialogSurface>
        ...
        {(!!cancelLabel || !!acceptLabel) && (
          <DialogFooter>
            {!!footer && { footer }}
            {!!cancelLabel && <DialogFooterButton cancel>Decline</DialogFooterButton>}
            {!!acceptLabel && <DialogFooterButton accept>{acceptLabel}</DialogFooterButton>}
          </DialogFooter>
        )}
      </DialogSurface>
      <DialogBackdrop />
