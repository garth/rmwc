  ...
  const Component = props => {
    ...
    const defaultClassNames =
      typeof opts.classNames === 'string'
        ? [opts.classNames]
        : typeof opts.classNames === 'function' ? opts.classNames(props) : opts.classNames || [];

    const themeClasses = props.theme
      ? (!Array.isArray(props.theme) ? [props.theme] : props.theme).map(t => `mdc-theme--${t}`)
      : [];

    const classes = classNames(...defaultClassNames, ...themeClasses, className);
